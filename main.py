# Original code from:
# <https://github.com/reprappro/Software/blob/master/pronterface/gcoder.py>
#
#!/usr/bin/env python
...
# Update to runtime 3.8
# Extract extrution length per time
# Export given data to a file
...
class Line(object):
    ...
    __slots__ = ('x', 'y', 'z', 'e', 'f', 'i', 'j',
                 ...
                 'current_tool',
                 'gcview_end_vertex', 'duration','extruded')
...
class Layer(list):
    ...
    __slots__ = ("duration", "z", "lines")

    def __init__(self, true_lines, z=None, lines=[]):
        super(Layer, self).__init__(true_lines)
        self.z = z
        self.lines = lines
...
class GCode(object):
    ...
    def _preprocess(self, lines = None, build_layers = False,
                ...
                if build_layers:
                    ...
                    if cur_z != prev_z:
                        ...
                        if base_z != prev_base_z:
                            ...
                            new_layer = Layer(cur_lines, base_z, cur_lines)
        ...
        if build_layers:
            if cur_lines:
                new_layer = Layer(cur_lines, prev_z, cur_lines)
                new_layer.duration = totalduration - layerbeginduration
                layerbeginduration = totalduration
                all_layers.append(new_layer)
                if cur_layer_has_extrusion and prev_z not in all_zs:
...
def main():
    ...
    lines = []
    file_name = "C:/Users/jnote/Downloads/Dragon 2.5_stl.gcode"
    ...
    f = open("C:/Users/jnote/OneDrive/Desktop/test.dat", "a")
    f.seek(0)
    f.truncate()

    total_e = 0
    total_duration = 0
    for i in gcode.all_layers:
        for j in i.lines:
            ...
            #print(("%.2f" % total_duration) + " " + ("%.2f" % total_e))
            f.write(("%.2f" % j.duration).zfill(10) + "\t" + ("%.2f" % j.extruded).zfill(10) +
                   "\t" + ("%.2f" % j.duration).zfill(5) + "\t" + j.raw + "\n")
